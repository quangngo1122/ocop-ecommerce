...
export default function AdminSellerPage() {
  const { data, loading } = useQuery(SELLERS_QUERY);
  const navigate = useNavigate();
  const [viewMode, setViewMode] = useState("table");
  // Lọc chỉ lấy user có role là "seller"
  ...
  const statusCounts = sellers.reduce(
    (acc, s) => {
      acc.all += 1;
  ...
  const filteredSellers = sellers.filter((s) => {
    ...
    if (match && statusTab !== "all") {
      match = statusTab === "active" ? s.isActive : !s.isActive;
    }
    return match;
  });
    ...
    <div className="relative">
      ...
      <div className="p-6">
        <div className="flex items-center gap-2 mb-6">
          <span className="w-1 h-6 bg-blue-500 rounded-sm inline-block" />
        ...
        <div className="justify-between items-center mb-6">
          <ExportButtons
            ...
            fileName="sellers_export"
            excelLabel="Excel"
            csvLabel="CSV"
            title="Admin | Quản Lý Người Bán"
          />
        </div>
        ...
        {/* <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {STATUS_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              className={`px-4 py-2 rounded border transition-all text-base font-medium ${
                statusTab === opt.value
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {STATUS_OPTIONS.map((opt) => (
            // <button
            //   key={opt.value}
            //   className={` cursor-pointer px-4 py-2 rounded border transition-all text-base font-medium ${
            //     statusTab === opt.value
            //       ? "bg-cyan-100 border-cyan-400 text-cyan-700"
            //       : "bg-gray-100 border-gray-200 text-gray-700 hover:bg-gray-200"
            //   }`}
        ...
        <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1">
          <div className="flex items-center gap-2 mb-3">
            <span>Số người bán mỗi trang:</span>
            <select
              value={
                itemsPerPage === filteredSellers.length ? "all" : itemsPerPage
              }
        ...
        <div className="overflow-x-auto bg-white rounded-lg shadow overflow-hidden border border-gray-200">
                  ...
                  >
                    ...
                    <td className="px-2 py-4 border text-center border-gray-300">
                      <button
                        ...
                        title="Xem chi tiết"
                        onClick={
                          () => navigate(`/admin/seller/detail/${seller._id}`)
                          // navigate(`/admin/user/detail/${seller.id}`)
                        }
                      >
                        <VisibilityIcon />
                      </button>
          ...
          <div className={viewMode === "card" ? "block" : "hidden"}>
            <CardView
              data={currentSellers}
              getImage={(s) => s.avatar}
              getTitle={(s) => s.fullName}
              onViewDetail={(s) => navigate(`/admin/seller/detail/${s._id}`)}
              fields={[
                { label: "Email", key: "email" },
                { label: "Vai trò", key: "role" },
                // {
                //   label: "Trạng thái",
                //   render: (s) => (s.isActive ? "Hoạt động" : "Khoá"),
                // },
                {
                  label: "Trạng thái",
                ...
                { label: "Ngày tạo", render: (s) => formatDate(s.createdAt) },
              ]}
            />
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mt-4">
          <div className="text-sm text-gray-700">
            Trang {currentPage} / {totalPages}. Người bán từ{" "}
            {(currentPage - 1) * itemsPerPage + 1} đến{" "}
            {Math.min(currentPage * itemsPerPage, filteredSellers.length)} trong
            tổng cộng {filteredSellers.length} người bán
          </div>
          {totalPages > 1 && (
